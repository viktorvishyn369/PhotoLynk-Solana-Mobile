...
stop_existing_service() {
  ...
  # Extra: kill any stale node server listening on 3000 (fallback)
  if command -v ss >/dev/null 2>/dev/null; then
    PIDS2=$($SUDO ss -ltnp 2>/dev/null | awk '$4 ~ /:3000$/ {print $7}' | sed 's/.*pid=\\([0-9]*\\).*/\\1/' | sort -u | tr '\n' ' ')
    if [ -n "$PIDS2" ]; then
